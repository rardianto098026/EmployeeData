...
namespace EmployeeData.Repository
{
    public class Common
    {
        ...
        public static string SqlDate3(string value)
        {
            var SqlDate = string.IsNullOrEmpty(value) ? "" : Convert.ToDateTime(value).ToString("MM/dd/yyyy");
        ...
        public static string GetEmployeeDetail(string EmployeeID, string param)
        {
            ...
            {
                DataTable dt = new DataTable();
                dt = ExecuteQuery("Select * from hrattendance.dbo.mst_employee Where [EmployeeID] = '" + EmployeeID + "'");
